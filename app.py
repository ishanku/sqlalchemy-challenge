...
def precipitation():
    #getPrecipitation = session.query(Measurement.date, Measurement.prcp).\
    ...
    #precipitation_dictionary = []
    def ConvertToDictionary(Tup,Dicty):
        for first,second in Tup:
            Dicty.setdefault(first,[]).append(second)
        return Dicty
    #ConvertToDictionary(getData,precipitation_dictionary)
    #for row in getData:
    #    date_dictionary = {}
    #    date_dictionary[row.date] = row.prcp
    #    precipitation_dictionary.append(date_dictionary)
